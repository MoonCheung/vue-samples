...
export default {
  ...
  methods: {
    success: function (pos) {
      // 由于笔记本电脑一般不带速度及方向传感器，从结果中可以看到返回值中heading及speed键值均为null
      let crd = pos.coords;
      console.log('当前位置:');
      console.log('纬度: ' + crd.latitude);
      console.log('经度: ' + crd.longitude);
      console.log('或多或少:' + crd.accuracy + '米.');

      this.$refs.speedVal.innerHTML = crd.speed;
      this.$refs.arrows.style.transform = `rotate(${crd.heading}deg)`;

      //手动赋值演示可视化结果
      this.$refs.speedVal.innerHTML = 12;
      this.$refs.arrows.style.transform = `rotate(30deg)`;
    },
    error: function (err) {
